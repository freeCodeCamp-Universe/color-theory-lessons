...
- Identify a screen-reader walkthrough whenever accessible names or descriptions, roles, values, states, live regions, focus movement, quiz feedback, or stage transitions change.
...
Record the browser, viewport, and keyboard flow in the pull request. Screen-reader walkthroughs are deployment prerequisites, not pull request merge prerequisites, unless the pull request's issue explicitly requires one. While #107 is open, add each required walkthrough to #107. After #107 closes, create a scoped issue for each new required walkthrough. Before deploying, complete the batch of walkthroughs tracked by the active issue and record the screen reader, browser and operating-system versions, learner flow, observed announcements, outcome, and unresolved findings. Keep visual-description findings within #53 and general structure, interaction, keyboard, focus, zoom, and reflow findings within #54.
