  ...
  quizItems: [
    {
      ...
      choices: [
        { stableId: 'a-single-mockup-is-sufficient-if-the-palette-looks-good', label: 'A single mockup is sufficient if the palette looks good', isCorrect: false, explanation: 'A palette can look great in one context and fail in others — dark mode, charts, and CVD simulation all exercise different roles.' },
        { stableId: 'different-contexts-exercise-different-roles-weaknesses-invisible', label: 'Different contexts exercise different roles — weaknesses invisible in one view may appear in another', isCorrect: true, explanation: 'A stress test across modes, chart views, alerts, and CVD simulation reveals problems that no single mockup can expose.' },
        { stableId: 'clients-prefer-seeing-multiple-mockups', label: 'Clients prefer seeing multiple mockups', isCorrect: false, explanation: 'Client preference is not the reason — functional quality is.' },
        { stableId: 'each-context-requires-a-completely-different-palette', label: 'Each context requires a completely different palette', isCorrect: false, explanation: 'A well-built system uses one role set across all contexts — the test verifies that it holds up.' },
      ],
    },
    {
      id: 'q2',
      ...
      choices: [
        { stableId: 'whether-all-hex-values-are-unique', label: 'Whether all hex values are unique', isCorrect: false, explanation: 'Multiple roles can share a hex value — uniqueness is not the goal of a consistency audit.' },
        { stableId: 'whether-roles-and-tokens-are-applied-consistently-one-role-per-m', label: 'Whether roles and tokens are applied consistently — one role per meaning, and changes propagating across contexts', isCorrect: true, explanation: 'Consistency means each role has one clear meaning, token references stay intact, and fixes propagate uniformly across the product.' },
    ...
    },
    {
      ...
      choices: [
        ...
        { stableId: 'whether-the-colors-look-good-in-a-brand-presentation', label: 'Whether the colors look good in a brand presentation', isCorrect: false, explanation: 'Brand presentation is one context — a complete review covers many more.' },
        { stableId: 'whether-the-design-team-likes-the-hues', label: 'Whether the design team likes the hues', isCorrect: false, explanation: 'Team preference is not the same as system robustness.' },
        { stableId: 'hierarchy-readability-semantic-clarity-dark-mode-chart-readabili', label: 'Hierarchy, readability, semantic clarity, dark mode, chart readability, and CVD robustness', isCorrect: true, explanation: 'A complete review covers all contexts, states, modes, and accessibility dimensions.' },
      ],
    },
  ],
  keyPoints: [
