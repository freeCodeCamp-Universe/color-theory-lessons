...
export const lesson6_5: LessonConfig = {
  ...
  steps: [
    ...
    { text: 'In the chart tuner, adjust series colors for a bar chart and a line chart. A CVD simulation button shows the chart under deuteranopia. A similarity warning appears when series become too close. Your challenge: make both charts clear in normal view and under simulation.' },
  ],
  challenge: {
  ...
  quizItems: [
    {
      ...
      choices: [
        { stableId: 'always-sequential-palettes-are-more-accessible', label: 'Always — sequential palettes are more accessible', isCorrect: false, explanation: 'Sequential palettes are not universally superior — they are appropriate for ordered data only.' },
        { stableId: 'when-data-has-a-meaningful-order-such-as-temperature-quantity-or', label: 'When data has a meaningful order — such as temperature, quantity, or severity level — and the goal is to show progression', isCorrect: true, explanation: 'Correct. Sequential palettes encode magnitude — they should not be used for unordered categories.' },
    ...
    },
    {
      ...
      choices: [
        { stableId: 'red-and-green-are-always-confusing-regardless-of-cvd', label: 'Red and green are always confusing regardless of CVD', isCorrect: false, explanation: 'Red and green are distinct to most viewers — the risk is specifically under CVD conditions.' },
        ...
        { stableId: 'under-protan-or-deutan-cvd-red-and-green-may-look-similar-users-', label: 'Under protan or deutan CVD, red and green may look similar — users cannot tell which is which', isCorrect: true, explanation: 'Correct. Red-green is the most common CVD pair to avoid when meaning depends on the distinction.' },
        { stableId: 'red-is-always-too-dark-for-charts', label: 'Red is always too dark for charts', isCorrect: false, explanation: 'Red\'s lightness depends on the specific value used — it is not inherently too dark.' },
      ],
    },
    {
      id: 'q3',
      ...
      choices: [
        { stableId: 'use-five-different-shades-of-one-hue', label: 'Use five different shades of one hue', isCorrect: false, explanation: 'Shades of one hue are hard to distinguish when used in multi-series charts.' },
        { stableId: 'add-direct-data-labels-to-each-series-so-the-legend-is-not-the-o', label: 'Add direct data labels to each series so the legend is not the only reference for identification', isCorrect: true, explanation: 'Direct labels work even when simulation makes hues similar.' },
        ...
        { stableId: 'use-a-table-instead-of-a-chart', label: 'Use a table instead of a chart', isCorrect: false, explanation: 'Tables serve a different purpose — they are not a substitute for chart accessibility.' },
      ],
    },
  ],
  keyPoints: [
