...
- **Unit 1**: Seeing and Describing Color (6 lessons, milestone, tools: before-after, slider-explore, contrast-checker, temperature-sorter, color-wheel, additive-sort)
...
All 6 units are fully built and wired. Milestones use mixed-part capstones (interactive challenges + short quizzes) with point-based pass thresholds. The `/capstone` route redirects to `milestone-6`.
...
- When creating a pull request, investigate any problems found outside the scope of its issue. Search the existing issues first. If an existing issue covers the problem, add a note to that issue. Otherwise, create a new issue.
